...
./.github/scripts/dashboard_check_repos_status_curl.sh $token calypsonet keyple-plugin-cna-arrive-android-jvm-lib
if [ $? -eq 0 ]; then
  exit 0
fi
...
./.github/scripts/dashboard_check_repos_status_curl.sh $token calypsonet keyple-plugin-cna-paragonid-gen5xx-pcsc-jvm-lib
if [ $? -eq 0 ]; then
  exit 0
fi
